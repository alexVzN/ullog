...
Record::Record(char* buffer, uint16_t bufferSize, int line, const char* fileName, uint16_t fileNameSize,
               const char* func, uint16_t funcNameSize, uint32_t timestamp_us, ISink& sink)
    : buffer_(buffer), buffer_size_(bufferSize), sink_(sink) {
    auto width = std::max(0, (int)(MIN_HEADER_SIZE - 2) - 1 - TIME_S_WIDTH - 1 - TIME_US_WIDTH - 1 - 1 -
                                 fileNameSize - 2 - funcNameSize - 2);
    auto res = snprintf(buffer_, buffer_size_, "[%4d.%06d][%s::%s::%-*d] ",
                        int(timestamp_us / 1000000), int(timestamp_us % 1000000),
                        fileName, func, width, line);
